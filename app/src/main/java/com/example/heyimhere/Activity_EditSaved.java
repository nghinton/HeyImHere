...
public class Activity_EditSaved extends AppCompatActivity implements DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
    ...
    public void createMessage(String message, String receiver) {
        ...
        // Create the intent for the new alarm and insert the message
        Intent intentNew = new Intent(getApplicationContext(), AlarmReceiver.class);
        intentNew.putExtra("MESSAGE_ID", mMessageViewModel.insert(newMessage));
        PendingIntent newAlarmIntent = PendingIntent.getBroadcast(getApplicationContext(), 0, intentNew, 0);

        // Use alarm manager to set the new alarm
